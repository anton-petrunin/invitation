...
Let us take $2^n$ copies of $P$ indexed by an $n$-bit array of 0s and 1s.
...
Given a $k$-dimensional simplex $\<v_{i_0},\dots,v_{i_k}\>$ in $\spc{S}$, let us mark the $(k\z+1)$-dimensional faces in $\square^N$ (there are $2^{N-k}$ of them) that are parallel to the coordinate $(k+1)$-plane spanned by $e_{i_0},\dots,e_{i_k}$.
