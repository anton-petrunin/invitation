...
Recall that metric on the cone \index{cone}\emph{cone} $\spc{V}=\Cone\spc{U}$ is defined via cosine rule.
...
The space $\spc{P}$ is locally $\CAT(0)$ if and only if the link of each simplex in $\tau$ has no closed local geodesic shorter than $2\cdot\pi$.
