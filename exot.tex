...
$\modtrig(pxy)_{\EE^2}$ and $\modtrig(qxy)_{\EE^2}$. Note however that they need not lie in the same 2-plane.
...
This gives $\CAT(0)$ comparison for the quadruple $p,q,x,y$ in $\spc{U}\times \spc{V}$.
\qeds
...
a (spherical) \index{polyhedral space}\emph{polyhedral space} 
...
It immediately follows from the construction of $\square_{\spc{S}}$ that the link of any vertex of $\spc{Q}$
is isometric to  ${\spc{S}}$
equipped with the all-right spherical metric. To see that $\square_{\spc{S}}$ is connected observe that it must contain the whole 1-dimensional skeleton of $\square^{N}$ which is connected.
...
\begin{subthm}{cube-infty} A cube complex $Q$ equipped with the $\ell^\infty$-metric is injective, i.e. for any  subset $Y\subset Q$ any 1-Lipschitz map from $Y$ to another metric space $\spc{Z}$ can be extended to a 1-Lipschitz map from $Q$ to $\spc{Z}$.
\end{subthm}
