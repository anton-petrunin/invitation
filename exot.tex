...
construct two model triangles $\trig{\tilde p_1}{\tilde x_1}{\tilde y_1}\z=\modtrig(p_1x_1y_1)_{\EE^2}$
...
Recall that suspension is a spherical analog of the cone construction; see Section~\ref{sec:constructions}.
...
%this needs to be reformulated.  If $\spc{U}$ is $CAT(1)$ it doesn't imply that a general neighborhood of the north pole in the suspension is also  $CAT(1)$. 
\end{thm}
...
By a \index{triangulation of a polyhedral space}\emph{triangulation} of a polyhedral space, 
...
If $\spc{P}$ is an $m$-dimensional polyhedral space (Euclidean or spherical),
...
A small neighborhood of $p$ is isometric to a neighborhood of the tip of $\Cone\Sigma_p$ (or $\Susp \Sigma_p$ if $\spc{P}$ is spehrical).
...
is called \index{flag complex}\emph{flag} if whenever $\{v_0,\z\ldots,v_k\}$
is a set of distinct vertices of $\mathcal{S}$
that are pairwise joined by edges, then the vertices $v_0,\ldots,v_k$
...
\[
\mangle\hinge{v_1}{v_2}{v_3}=
\mangle\hinge{v_2}{v_3}{v_1}=
\mangle\hinge{v_3}{v_1}{v_2}=
\pi.
\]
Equivalently,
\begin{clm}{}\label{clm:3pi/2}
The product
of the geodesics $[v_1v_2]$, $[v_2v_3]$, and $[v_3v_1]$
...
and runs in $\Star_v$ (figure~\ref{pic-1000}).
...
Given a $k$-dimensional simplex $\langle v_{i_0},\ldots,v_{i_k}\rangle$ in $\spc{S}$, let us mark the $(k\z+1)$-dimensional faces in $\square^N$ (there are $2^{N-k}$ of them) that are parallel to the coordinate $(k+1)$-plane spanned by $e_{i_0},\ldots,e_{i_k}$.
...
denote this edge by~$e$ (figure~\ref{pic-1100}).
...
Observe that $\tilde G$ is homeomorphic to the plane; indeed, $\tilde G$ is
...
For more on $\CAT(0)$ cube complexes, see the draft by Montserrat Casals-Ruiz, Mark Hagen, and Ilya Kazachkov \cite{casals-hagen-kazachkov} and the references therein.
