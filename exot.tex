...
This lecture describes a set of rules for gluing Euclidean cubes that produce a 
locally $\CAT(0)$ space and uses these rules to construct exotic examples of aspherical manifolds.
...
\[\dist{p}{q}{\spc{X}}\le \dist{\tilde p}{\tilde z}{\EE^4}+\dist{\tilde z}{\tilde q}{\EE^4}.\]
This gives $\CAT(0)$ comparison for the quadruple $p,q,x,y$ in $\spc{U}\z\times \spc{V}$.
\qeds

Recall that the metric on the cone \index{cone}\emph{cone} $\spc{V}=\Cone\spc{U}$ is defined via the cosine rule.
...
Applying $\CAT(0)$ comparison for these points leads to $\CAT(1)$ comparison for the quadruple $ p'$, $q'$, $ x'$, $ y'$ in $\spc{U}$.
...
and the following statement is a direct analog of \ref{ex:cone+susp};
...
The $\CAT(0)$ case is done in exactly the same way except we need to use Proposition~\ref{ex:cone+susp} and the first part of Corollary~\ref{cor:closed-geod-cat} on the last step.
\qeds
...
Theorem \ref{thm:PL-CAT} gives is a good-looking description of polyhedral $\CAT(\kappa)$ spaces,
...
The $\CAT(0)$ property of a cube complex admits interesting (and useful) geometric descriptions if one exchanged the $\ell^2$-metric to natural $\ell^1$ or $\ell^\infty$ metrics on each cube.
...
\begin{subthm}{cube-infty} A cube complex $Q$ equipped with the $\ell^\infty$-metric is injective; that is, for any subset $Y\subset Q$ any 1-Lipschitz map from $Y$ to another metric space $\spc{Z}$ can be extended to a 1-Lipschitz map from $Q$ to $\spc{Z}$.
\end{subthm}

\begin{subthm}{cube-1} A cube complex $Q$ equipped with the $\ell^1$-metric is \index{median space}\emph{median}.
 The latter means that for any three points $x,y,z$ there is a {}\emph{unique} point $m$ (it is called the \index{median}\emph{median} of $x$, $y$, and $z$) and a choice of geodesics $[xy], [xz], [yz]$ such that $[xy]\ni m$, $[xz]\ni m$ and $[yz]\ni m$.
\end{subthm}
\end{thm}
