...
Given an integer $m\ge 4$ there is a $(m-1)$-dimensional smooth homology sphere $\Sigma^{m-1}$ that is not simply connected, and bounds a contractible smooth compact $m$-dimensional manifold~$\spc{W}$.
\end{thm}
...
In dimension 3 it was showen by
...
They constructed a closed smooth 4-dimensional manifold $M$ with universal cover $\tilde M$ diffeomorphic to $\RR^4$, such that $M$ admits a polyhedral metric which is locally $\CAT(0)$, but does not admit a Riemannian metric with nonpositive sectional curvature.
