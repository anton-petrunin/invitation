...
Let $K\subset \EE^m$ be closed connected set that admits a smooth parametrization by a closed $m$-ball.
...
Assume there is a Lipschitz homeomorphism from closed ball in $\EE^3$ to $K$.
Then $K$ with the induced length metric is $\CAT(0)$ if and only if the interior of $K$ is two-convex.
\end{thm}

\parit{Proof.}
Set $\Omega=\Int K$ by the domain invariance theorem, $\Omega$ is homeomorphic to an open ball; in particular it is simply connected.
...
we get that $\CAT(0)$ comparison holds for $x^1,x^2,x^3,x^4$ in $\Omega$.
