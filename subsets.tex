...
The answers are given below, but it is instructive to think before reading further.
...
Then by Proposition~\ref{prop:two-cove+smooth}, there is a triangle $\trig xyz$ in $K$ that is null-homotopic in $K$, but the solid triangle $\Delta=\Conv\{x,y,z\}$ does not lie in $K$ completely.
...
Given an array of values $s^1<\ldots<s^k$ in $[-\eps,\eps]$,
set $V^i=V_{s^i}$ and
consider the unions 
\[W^i=V^1\cup\ldots\cup V^i\]
equipped with the induced length metric.

Note that the array $(s^1,\ldots,s^k)$ can be chosen in such a way that
...
\parit{Induction step:} Assume that $W^i$ is $\CAT(0)$.
...
By the Reshetnyak gluing theorem (\ref{thm:gluing}), $W^{i+1}$ is $\CAT(0)$.
...
Corollary~\ref{cor:shefel} will provide a more general statement in 3-dimensional Euclidean space.
...
First we prove its polyhedral analog, then we prove some properties of two-convex hulls in 3-dimensional Euclidean space and only then do we prove the general statement.
...
\begin{thm}[!]{Very advanced exercise}\label{ex:convex+saddle+broken=>PL}
Let $\bar S$ be the closure of $S$.
Denote by $\partial\bar S$ the boundary of $\bar S$ in $\partial \Omega'$.

\begin{subthm}{ex:convex+saddle+broken=>PL:a}
Show that (1) and (2) imply that any point $p\in S$ lies in a line segment in $\bar S$ with ends in $\partial\bar S$ or in a polygon in $\bar S$ with vertices in~$\partial\bar S$.
\end{subthm}
...
Fix $\eps>0$.
Choose six polygonal lines in $\tilde \Omega$ connecting all pairs of points $x^1,x^2,x^3,x^4$, where the length of each polygonal line is at most $\eps$ bigger than
...
admits a lift $\iota\:X\hookrightarrow \tilde K'$, and $\iota$ almost preserves the distances between the points $x^1,x^2,x^3,x^4$;
namely 
\begin{align*}
\dist{\iota(x^i)}{\iota(x^j)}{\tilde K'}\lessgtr \dist{x^i}{x^j}{\Int P}\pm\eps.
\end{align*}

Since $\eps>0$ is arbitrary and $\CAT(0)$ comparison holds in $\tilde K'$,
we get that $\CAT(0)$ comparison holds for $x^1,x^2,x^3,x^4$ in $\tilde \Omega$.
...
A generalization of this result is proved by Alexander Lytchak and Stefan Wenger \cite[Proposition 12.1]{lytchak-wenger}; this paper also contains a far-reaching application.
...
By a result of Alexander Lytchak and Stefan Wenger \cite{lytchak-wenger}, Shefel's conjecture would follow if the constant would be optimal; that is, $C=\tfrac{1}{4\cdot\pi}$.
