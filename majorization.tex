...
Notice that the majorization theorem implies isoperimetrical inequality
--- \textit{any curve of length $2\cdot \pi\cdot r$ in a $\CAT(0)$ length space spans a disc of are at most $\pi\cdot r^2$}.
Almgren's inequality \cite{almgren} provides an analogous statement for all dimensions and codimensions.
For the $\CAT(0)$ spaces this question is open.
Even in the smooth case, it is solved only if dimension $\le 4$ and codimension $1$ \cite{kleiner-1992,croke}.
