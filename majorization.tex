...
=\dist{x}{z}{}+\delta\cdot\eps+o(\eps),\]
where $\delta=-\dist{p}{z}{}\cdot\cos\hinge zpx>0$.
...
Clearly $\beta_n=\beta$.
Sequentially applying Alexandrov's lemma (\ref{lem:alex}), we get that each of the polygonal lines $\beta_{n-1}, \beta_{n-2},\dots,\beta_1$ is concave with respect to $p$.
...
Further, $[x^1\dots x^n ]$ (for $n\ge 3$) will denote a polygon
with edges $[x_1x_2],\dots,[x_{n-1}x_n]$, and $[x_n x_1]$.
...
We claim that any closed $n$-gon $[x^1x^2 \dots x^n]$ in a $\CAT(0)$ space  is majorized by a convex polygonal region \[R_n=\Conv[\tilde x^1\tilde x^2\dots\tilde x^n]\]
...
Let $P=[x^1\dots x^{n}]$ be a polygon in a geodesic $\CAT(0)$ space $\spc{U}$.
Suppose $\tilde P=[\tilde x^1\dots \tilde x^{n}]$ is a convex  polygon in $\EE^2$
such that 
\[
