...
We claim that any closed $n$-gon $[x^1x^2 \ldots x^n]$ in a $\CAT(0)$ space $\spc{U}$ is majorized by a convex polygonal region
\[R_n=\Conv[\tilde x^1\tilde x^2\ldots\tilde x^n],\]
under a map $F_n$ that sends $\tilde x^i\mapsto x^i$ for each $i$.
...
Since $F_{n-1}$ and $\dot F$ agree on $[\tilde x^1 \tilde x^{n-1}]$, we may define $F\:R\to\spc{U}$ by
\[
F(x)=
\begin{cases}
