...
It is instructive to solve the following exercises before diving into the proof of the theorem.
...
Prove the following generalization of the arm lemma.
\end{thm}
...
The statement holds for any geodesic $\CAT(0)$ space but one need to be more careful \cite[9.56]{alexander-kapovitch-petrunin-2025}.
...
\begin{thm}{Advanced exercise}\label{ex:isometric-majorization}
Let $\spc{U}$ be a geodesic $\CAT(0)$ space
and $\alpha\:[0,\ell]\to\spc{U}$ be a closed curve with arclength parametrization.
Assume there is a unit-speed closed convex curve $\tilde \beta\:[0,\ell]\to\EE^2$ such that 
\[\dist{\alpha(t_0)}{\alpha(t_1)}{\spc{U}}=\dist{\tilde \beta(t_0)}{\tilde \beta(t_1)}{\EE^2}\]
for any $t_0$ and $t_1$.
Show that there is a distance-preserving map $F\:\Conv\tilde \beta\to \spc{U}$
such that $F\:\tilde \beta(t)\mapsto \alpha(t)$ for any $t$.
\end{thm}
...
Another proof can be built on generalized Kirszbraun's theorem \cite{lang-schroeder,alexander-kapovitch-petrunin-2011,alexander-kapovitch-petrunin-2025}, but it works only for complete spaces.
...
Consider be a closed rectifiable curve $\alpha$ in a $\CAT(0)$ space $\spc{U}$.
...
Notice that the majorization theorem implies isoperimetrical inequality for $\CAT(0)$ metrics on the plane.
Moreover, it implies the generalized isoperimetrical inequality in the style of Frederick Almgren \cite{almgren}: \textit{any curve of length $2\cdot \pi\cdot r$ in a $\CAT(0)$ length space spans a disc of area at most $\pi\cdot r^2$}.

It is known that isoperimetrical inequality holds for $\CAT(0)$ Riemannian manifolds of dimension 3 and 4.
First the 4-dimensional case was proved by Christopher Croke \cite{croke},
and after that Bruce Kleiner proved the 3-dimensional case \cite{kleiner-1992}.
Each of these two papers is a masterpiece.
Despite many attempts, the isoperimetrical inequality for $\CAT(0)$ spaces remains open in all other dimensions and codimensions. 
