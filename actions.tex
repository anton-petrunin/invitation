...
In the definition of spaces with curvature $\ge1$, 
...
Let $G$ is a subgroup of the isometry group of $\spc{R}$.
...
If $G$ has closed orbits then according to Theorem~\ref{thm:CBB/G}, the orbit space $\spc{R}/G$
is a therefore  group $\CBB{}{\kappa}$ space.
...
Given a point $p\in \spc{R}$ we denote by $[p]_G$ its orbit in $\spc{R}/G$;
also given a set $X\subset\spc{R}$ we denote by $[X]$ the subset of all orbits in $\spc{R}/G$ with a point in $X$.

Then $\spc{R}^G/G$ is an extremal set in $\spc{R}/G$.
\end{thm}
