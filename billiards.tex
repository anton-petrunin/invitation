...
Show that any ball in a $\Cat{}{0}$ proper length space forms a convex set.

Analogously, show that any ball of radius $R<\tfrac\pi2$ in a $\Cat{}{1}$ proper length space  forms a convex set.
\end{thm}
...
The gluing theorem (\ref{thm:gluing}) holds for the class of $\Cat{}{0}$ geodesic spaces 
which by Exercise~\ref{ex:product} includes all $\Cat{}{0}$ complete length spaces.
It also admits natural generalization to $\Cat{}{\kappa}$ spaces, 
see our book \cite{AKP} for details.%??? MORE REFS???

The idea described above has number of applications to the billiards.
...
Note that the walls in the puff pastry plays a key role in the proof
despite the fact that that trajectories never come into the walls.
In a similar fashion, puff pastry was used by Alexander and Bishop in \cite{a-b:warped}
to find the upper curvature bound for the warped products.
