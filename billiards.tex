...
Here we introduce the notion of thin triangles and
use it to define of $\Cat{}{}$ spaces. 
...
We say that the space $\spc{X}$ is $\Cat{}{0}$ if 
%S
it is a geodesic space and
% 
all its triangles are thin.
...
while $\Cat{}{1}$ spaces will play a role only  due to the following statement.
...
\parit{Proof.} Given 4 points $p^1,p^2,q^1,q^2$ in a $\Cat{}{0}$ space $\spc{U}$, 
...
Recall that a surface formed by one parameter family of line segments is called \emph{ruled surface};
that is, it admits a parametrization $f(u,v)=a(u)+b(u)\cdot v$.
The following Exercise is a classical theorem prved by  Alexandrov in \cite{alexandrov:ruled}.

\begin{thm}{Exercise}
Show that any Lipschitz ruled surface in the Euclidean space 
equipped with induced length metric forms a $\Cat{}{0}$ space.
\end{thm}
...
\begin{thm}{Exercise}
Show that in any $\Cat{}{0}$ space any closed locally convex set is convex.
\end{thm}
...
Suppose 
$\spc{U}^1$ and $\spc{U}^2$ are $\Cat{}{0}$ spaces 
with isometric 
%S
%closed 
complete convex
...
complete
%
space which has two closed convex sets $\spc{U}^1,\spc{U}^2\subset\spc{W}$ such that
$\spc{U}^1\cup\spc{U}^1=\spc{W}$ and $\spc{U}^1$, $\spc{U}^2$ are $\Cat{}{0}$.
...
%S
First we carry out the proof supposing our space $\spc{W}$ is geodesic:

It suffices to show that any triangle $\trig {x^0}{x^1}{x^2}$ of perimeter $<2\cdot \varpi\kappa$ 
in $\spc{W}$ is thin.
...
%S
Returning to the general case, we complete the proof by showing $\spc{W}$ must be geodesic:

For $x^1\in\spc{U}^1$, $x^2\in\spc{U}^2$, we may choose a sequence $z^i\in A$ such that $\dist{x^1}{z^i}{} + \dist{z^i}{x^2}{}$ converges to $\dist{x^1}{x^2}{}$, 
and $\dist{x^1}{z^i}{}$ and $\dist{x^2}{z^i}{}$ converge. Since $A$ is complete, it suffices to show $z^i$ is a Cauchy sequence; in that case,  the limit point $p$ of $z^i$ satisfies  $\dist{x^1}{p}{} + \dist{p}{x^2}{}=\dist{x^1}{x^2}{}$, so the geodesics $[x^ 1p]$ in $\spc{U}_1$ and $[x^ 2p]$ in $\spc{U}_2$ form a geodesic $[x^1x^2]$ in $\spc{W}$.

Let $z$ be the midpoint of $[z^i z^j]$. For any  $\eps >0$, we may assume (taking  a subsequence and possibly relabeling $x^1$ and $x^2$) that for $i,j$ sufficiently large,
\[
\dist{x^1}{z}{}\ge \dist{x^1}{z^i}{} - \eps,\ \ 
\dist{x^1}{z}{}\ge \dist{x^1}{z^j}{} - \eps.
\]

Since triangle $\trig{x^1}{z^1}{z^2}$ is thin, the analogous inequalities hold for the Euclidean model triangle $\trig{\tilde x^1}{\tilde z^1}{\tilde z^2}$ and the midpoint  $\tilde z$  of $[{\tilde x^i}{\tilde x^j}]$.  
Moreover, $\dist{x^1}{z}{} \le \max\{\dist{x^1}{z^i}{}, \dist{x^1}{z^j}{}\}$, so  $\dist{x^1}{z}{}$ and $ \dist{x^1}{z^i}{}$ converge to the same limit. 
Therefore $\dist{\tilde z^i}{\tilde z^j}{}$ becomes arbitrarily small for $i,j$ sufficiently large; otherwise there would be a nondegenerate limit triangle $\trig{p}{q}{r}$ in the Euclidean plane with $\dist{p}{q}{} = \dist{p}{r}{} =\dist{p}{m}{}$, where $m$ is the midpoint of 
$[qr]$, a contradiction. Since $\dist{\tilde z^i}{\tilde z^j}{}=\dist{z^i}{z^j}{}$, then $z^i$ is a Cauchy sequence.  The theorem follows.
\qeds
...
\begin{thm}{Exercise} Let $\spc{U}$ be a $\Cat{}{0}$ space.
...
Assume that any dihedral angle containing $A\cap B$ with the edge passing through $A\cap B$ has measure at least $\alpha$.
