...
\end{thm}
%??? Should Exercises 2.2.11 - 2.2.13 be moved up to follow Corollary 2.2.6? S. Move it if you feel so --- I am neutral. A.
...
forms a convex set in $\spc{R}$. %%%???please check my English and remove this comment.
\end{thm}
...
that is, $\eps$ is the maximum angle between two generatrix lines of the cone.%%%???please check my English and remove this comment
...
\parit{Proof.}
The array $\bm{j}_\eps(n)=(j_1,\dots,j_N)$ from the indexes $1,\dots,n$ is constructed recursively.
...
the end-to-end convexity of the puff pastry for $\bm{j}_\eps(n+1)$
...
have $\eps$-wide corners.%%%???please check my English and remove this comment
...
see an other paper of Dmitri Burago, Serge Ferleger and Alexey Kononenko \cite{BFK-2}.
...
In \cite{hass},
Joel Hass constructed an example of Riemannian metric on 3-ball with negative curvature and concave boundary.
This example might put off your appetite in generalizing Collision Theorem --- while
locally such 3-ball looks as good as the billiards table in the theorem, the number of collisions is obviously infinite.%%%???please check my English and remove this comment.
