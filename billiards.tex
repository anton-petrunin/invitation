...
The following theorem was proved by Reshetnyak for proper spaces in \cite{reshetnyak:glue}.
...
%>>>>>>> 05815d2d9699d712a209a65888348851fe07213c
...
%S
Now, returning to the general case, we complete the proof by showing that  $\spc{W}$ really is a geodesic space:
...
Let $z^{ij}$ be the midpoint of $[z^i z^j]$. For any  $\eps >0$, we may assume (taking  a subsequence and possibly relabeling $x^1$ and $x^2$) that for $i,j$ sufficiently large,
\[
\dist{x^1}{z^{ij}}{}\ge \dist{x^1}{z^i}{} - \eps,\ \ 
\dist{x^1}{z^{ij}}{}\ge \dist{x^1}{z^j}{} - \eps.
\]

Since triangle $\trig{x^1}{z^i}{z^j}$ is thin, the analogous inequalities hold for the Euclidean model triangle $\trig{\tilde x^1}{\tilde z^i}{\tilde z^j}$ and the midpoint  $\tilde z^{ij}$  of $[{\tilde x^i}{\tilde x^j}]$.  
Moreover, $\dist{\tilde x^1}{\tilde z^{ij}}{} \le \max\{\dist{\tilde x^1}{\tilde z^i}{}, \dist{\tilde x^1}{\tilde z^j}{}\}$, so  $\dist{\tilde x^1}{\tilde z^{ij}}{}$ and $ \dist{\tilde x^1}{\tilde z^i}{}$ are arbitrarily close for for $i,j$ sufficiently large. 
But then $\dist{\tilde z^i}{\tilde z^j}{}$ becomes arbitrarily small for $i,j$ sufficiently large: otherwise there would be a nondegenerate limit triangle $\trig{\tilde x^1}{q}{r}$ in the Euclidean plane with $\dist{\tilde x^1}{q}{} = \dist{\tilde x^1}{r}{} =\dist{\tilde x^1}{m}{}$, where $m$ is the midpoint of 
