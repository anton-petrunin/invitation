...
The following exercise is a bit simpler, 
...
Then 
\[t\mapsto\dist{\geodpath_{[p^1q^1]}(t)}{\geodpath_{[p^2q^2]}(t)}{\spc{U}}\]
...
\begin{lpic}[t(-4mm),b(0mm),r(0mm),l(0mm)]{pics/dihedral-angle(1)}
\lbl[t]{2,5;$p$}
...
and Alexey Kononenko~\cite{BFK-2}, and with
Dimitri Grigoriev and Anatol Slissenko~\cite{BGS}.
...
It was show by Dmitri Burago and Sergei Ivanov \cite{burago-ivanov} that the number of collisions that may occur between $n$ identical balls in $\mathbb{R}^3$ grows at least exponentially in $n$.
