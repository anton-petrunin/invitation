...
The main motivating example for the definition of $\Cat{}{\kappa}$ spaces is the following.
\begin{thm}{Example}
Let $(M^n,g)$ be a Riemannian manifold. Let $d$ be the metric on $M$ induced by $g$. Then it easily follows from the Rauch comparison for Jacobi fields  and Proposition ~\ref{prop:thin=cat} below that $(M,d)$ has $\sec\le \kappa$ if an only if it is locally $\Cat{}{\kappa}$, i.e. for every point $p\in M$ all sufficiently  small balls around $p$ are  $\Cat{}{\kappa}$.
\end{thm}
