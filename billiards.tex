...
as long as any quadruple of points in the limit space can be arbitrary well approximated by  quadruples in the sequence of metric spaces. 
...
\begin{thm}{Proposition}\label{prop:convex-2-geos}
...
Show that the distance function $\dist{p}{}{}$ for any point $p$ in a  $\Cat{}{0}$ proper length space is a convex.
...
is called \index{end-to-end convex}\emph{end-to-end convex} 
if the union of the lower and the upper levels 
...
\begin{lpic}[t(-4mm),b(0mm),r(0mm),l(0mm)]{pics/puff-pastry(1)}
\lbl[t]{23,33,6;$\alpha$}
\lbl[b]{20,47,6;$\alpha$}
\lbl[b]{30,3,6;$\alpha$}
...
$\bigcap_{i\in F}A^i$ 
has $\eps$-wide corners.
In this case we may say briefly \index{$\eps$-wide corners}\emph{all intesections of $A^i$ have $\eps$-wide corners}.


\begin{thm}{Exercise}\label{ex:compact-walls}
Assume that the walls
of a billiard table $T$ 
