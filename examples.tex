...
Namely, for any plane curve $\gamma$ (\emph{not necessary simple}) which is also contractible in $K$ is contractible in the intersection of $K$ with the plane of $\gamma$.
...
In particular, the set $\hat\phi^{-1}(W)$ consists from the same curves as $\phi^{-1} (W)$ with the exception of $c'$.
...
Namely, we can say that $K$ is two-convex if the following condition holds: if a 1-dimensional cycle $z$ has support in the intersection of $K$ with a plane $W$ and bound in $K$ then it bound in $K\cap W$.

The obtained definition is equivalent to the one used above.
Unlike our definition it can be generalized to define $k$-convex sets in $\EE^m$ for $k>2$.
With this homological definition one can also avoid use of Papakyriakopoulos' Loop theorem, which proof is quite involved.
Despite all this, we used homotopic definition since it is easier to visualize.
