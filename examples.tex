...
%??? I don't quite see the point of this exercise. isn't it just about nonexistence of simple diangles which implies that all triangles are simple?V.
%??? I had the same reaction as Vitali to Sasha's exercise, was going to put it in and decided against it. But maybe its statement is sufficiently interesting to justify it. S.
...
Note that it is enough to consider only simple triangles $[xyz]$, 
...
%\cite{petrunin-shefel}.
% There is a ref to my paper with Stadler in the end of the chapter.
% but that does not treat 3-d case, so is misleading for the reader.  S.
...
% past tense is better here then but I'll leave it for Stephanie to reword the whole thing if necessary. V.
%actually people do use the present tense in this way, i.e. to describe the impression you get from reading a text. Both ways are fine, but "seems" puts more energy into the sentence so I slightly prefer it. Vitali, please change back if you aren't persuaded!!!
%%??? Fine with me. V.
...
(This may happen only on the first step, namely in the construction of $\Omega_1$ from $\Omega_0$, but we do not use this fact.)
...
We use our definition only for the open sets, for general sets it would be more natural to use \v{C}ech cohomology instead. %??? I don't get that. cohomology?!  how does one define two-convexity using Chech cohomology? V.

If a surface $S$ in $\EE^3$ forms a two-convex set 
then it is regarded as a \emph{saddle surface}.
...
The conjecture is open even for surfaces represented by a bi-Lipschitz embedding of a disc.
