...
then there is a point $p\in \mathfrak{C}\backslash\Conv_2\Omega$ lying on the maximal distance from $\Pi_n$.
...
Since $p$ lies on the maximal distance from $\Pi_n$,
...
Under the name \emph{$(n-2)$-convex sets}, two-convex sets were introduced by Mikhael Gromov in \cite{gromov:SaGMC}.
In addition to the discussed theorems, these sets appear as the maximal open sets of Riemannian manifolds with nonnegative or nonpositive sectional curvature.
It was first observed by Sergei Buyalo for nonpositive curvature \cite[Lemma 5.8]{buyalo} 
