...
we say that $p$ is a \index{positive critical point}\emph{positive critical point}
...
Note that by Proposition \ref{prop:convex-2-geos}, the distance function $f_0(x)=\dist{\Conv\gamma}{x}{\EE^m}$ is convex.  Therefore $f_0$ can be approximated by smooth strongly convex functions $f$ such that the restriction $f|_S$ is a Morse function.
...
\parit{Proof of Proposition \ref{prop:two-hull-open}.}
...
This theorem is a baby case of the main result of \cite{a-b-b:CBA-m-w-b}, which is briefly discussed at the end of the chapter. 
...
$K$ is a $\Cat{}{0}$ space if $K$ is \index{strongly two-convex set}\emph{strongly two-convex},
...
We sketch the proof from Bishop's note \cite{bishop:jordan} and suggest that  the details be finished as an exercise.
...
Consider the universal cover $K$ of $\EE^4$ infinitely branching along $\Pi_1$ and $\Pi_2$ and equip it with the induced length metric.
...
The following corollary is a generalization of Theorem \ref{thm:set-with-smooth-bry:CBA} for the three-dimensional Euclidean space.
...
A continuous function $f\:U\to\RR$ is called \index{saddle function}\emph{saddle} if for any linear function $\ell\:\RR^2\to\RR$ the difference 
$f-\ell$
...
Shefel's Theorem~\ref{thm:shefel} holds if the set $\Omega$ is formed by the interior of a polytope.
\end{thm}

The statement might look obvious, but the proof has a hidden obstacle.
%it has an underwater stone.
...
(For example, let $K=\{-|x|-|y|\le z\le |x|+|y|\}$ and let $p$ be the origin. Then $\Sigma_p$ is not simply connected and $\tilde K\to K$ branches at $p$.)
...
the central projection $\gamma'$ of $\gamma$ to $\Pi$ is defined and lies in $\Omega$. Moreover, it is contractible in $\Omega$.
%??? a picture here would be really helpful
...
In \cite{a-b-b:CBA-m-w-b} Alexander, Berg and Bishop gave the exact upper bound on Alexandrov's curvature for the Riemannian manifolds with boundary.
