...
Suppose $f\:\EE^m\to\RR$ is a smooth strongly convex function such that the restriction $f|_S$ is a Morse function.
...
If $p\in\Int K$, then it admits a neighborhood isometric to a subset of $\EE^m$ which is $\Cat{}0$.
...
By Reshetnyak gluing theorem, $W^{i+1}$ is $\Cat{}{0}$.
...
\end{clm}
Indeed, once this is proved we will know that $\tilde K$ is locally contractible. Hence any loop in $\tilde K$ is homotopic to a loop in $\tilde\Omega$ which is simply connected. Therefore $\tilde K$ is simply connected too.
%Is it obvious that $\tilde (K)$ is simply connected? 
