...
\parit{``If'' part.}
Recall that a smooth function $f\:\EE^m\to\RR$ is called \emph{strictly convex} if its Hessian is positive definite at each point.
%strongly convex means minimum eigenvalue \ge uniform positive constant.
...
Since $k_2\ge 0$ and the function $f$ is  strictly
 convex, 
...
is a continuous map from disc $\DD$ such that the boundary curve $f|_{\partial \DD}$ is not null-homotopic in $\partial M$.  Then there is an embedding 
...
If $a<b<c$, then $V_b\supset V_a\cap V_c$.
% should some explanation be given here why this inclusion holds?
\end{clm}
...
\end{clm}
%Indeed, once this is proved we will know that $\tilde K$ is locally contractible.
%Hence any loop in $\tilde K$ is homotopic to a loop in $\tilde\Omega$ which is simply connected.
%Therefore $\tilde K$ is simply connected too.
...
4 points $x^1,x^2,x^3,x^4\in\tilde \Omega$.
...
Choose a polytope $P$ in $\Omega$ such that the interior $\Int P$ contains the projections of all six broken lines and discs which contract all the cycles created by them (3 discs is enough).
...
By construction $\Int P$, the embedding $\Int P\hookrightarrow \Omega'$
...
With this homological definition one can also avoid the use the loop theorem, whose proof is quite involved.
