...
We use our definition only for the open sets, for general sets it would be more natural to use \v{C}ech cohomology instead. %??? I don't get that. cohomology?!  how does one define two-convexity using Chech cohomology? V. I am not sure how to formulate it, maybe just say ``proper homology theory'' --- in principle it can be done with cohomology, but it is not the point I want to make. A.
%%%??? By "proper" you mean "appropriate", right? I am still not sure which homology theory is right to take here. V.

If a surface $S$ in $\EE^3$ is saddle if each connected bounded component which any plane cuts from $S$ contains a point on the boundary of $S$.
If $S$ is locally described by a Lipschitz embedding then 
this condition equivalent to saying that $S$ is two-convex.
...
Also a counterexample could not admit an approximation by smooth saddle surfaces. 
Examples which do not admit such approximation are not known so far.
...
it discusses a variation of Shefel's conjecture with a $\Cat{}{0}$ ambient space and also a related result of Shefel in dimension 2. 
