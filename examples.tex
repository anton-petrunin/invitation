...
\parit{``If'' part.}
Recall that a smooth function $f\:\EE^m\to\RR$ is called \emph{strongly convex function}\emph{strongly convex} if its Hessian is
...
Since 
%$S$ is two-convex 
$k_2\ge 0$ and $f$ is strongly convex, 
...
Note that by Proposition \ref{prop:convex-2-geos}, the distance function \[f_0(x)\z=\dist{\Conv\gamma}{x}{\EE^m}\] 
is convex.
...
region whose boundary is a triangle $T$ with concave sides.
Equip the closure $S$ with the induced length metric.
...
The material below was inspired by work of Samuel Shefel, and developed by Anton Petrunin \cite{petrunin-shefel}. 
...
For example, if \[K=\set{(x,y,z)\in\EE^3}{|z|\le |x|+|y|\le 1}\] and $p$ is the origin then $\Sigma_p$ is not simply connected and $\tilde K\to K$ branches at $p$.
...
Suppose we already know that  $\Conv_2\Omega\supset\Omega_{n-1}$. Given a bounded component  $U$ of $\EE^3\backslash(\Pi_n\cup \Omega_{n-1})$, its boundary
%it touches 
%["touches" means "intersects"] 
 intersects $\Pi_{n}$ in
a bounded component  $\hat U$ of $\Pi_{n}\backslash \Omega_{n-1}$. Then there is a curve $\gamma$ in 
$W=\Pi_{n}$ 
...
Denote by $F_1,\dots,F_m$ the facets of $P$. By subdividing $F_i$ if necessary, we may assume that all $F_i$ are convex.
...
A 
%sectional direction 
%It seems this use of "section" is not widely understood.  Igor Belegradek sent me an angry e-mail about this.
2-plane tangent to the boundary
will be called concave if all the directions in it are concave.

Then $M$ is locally $\Cat{}{\kappa}$, where $\kappa$ is an upper bound of sectional curvatures of $M$ and  
concave 
sectional curvatures of $\partial M$. %in the concave directions.
\end{thm}
