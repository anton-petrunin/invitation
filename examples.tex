...
then the disc in $W$ bounded by $\gamma$ lies in $K$.
\end{thm}

Note that two-convex set does not have to be connected, nor simply connected. 
...
Fix a closed simple plane curve $\gamma$ 
...
The proof is taken from \cite[\S\textonehalf]{gromov:SaGMC}, but we through in some details.
%???added
...
Since $f$ is generic, we can assume that the sign is defined for all critical point's;
...
Indeed, we can construct a homotopy $\phi_t\:\DD\to K$ which decrease the maximum of $f\circ\phi$ on $\DD$ until in hangs on a critical point of $f|_S$.
This point can not be negative, otherwise its index would be at least 2.
If this point a positive then it is easy decrease the maximum a little by pushing disc from $S$.%%%???please check English and remove this comment
...
Note that the ``if'' part proves a bit stronger statement.
Namely for any plane curve $\gamma$ (not necessary simple) which is contractible in $K$ is contractible in the intersection of $K$ with the plane of $\gamma$.
...
then is null-homotopic in $\Omega\cap W$.
\end{thm}

\parit{Proof.}
Fix a closed plane curve which is null-homotopic in $\Omega$.
Since $\Omega$ is open we can assume that $\gamma$ as well as the disc $D$ which shrinks $\gamma$ in $\Omega$ lie in general position.
In particular $D$ intersects $W$ transvesally at all its interior points.

The plane $W$ cuts $D$ into components.
Note that one of these components, say $D'$, is a disc;
it lies on one side from $W$.

If the boundary of $D'$ is not contratable in $W$ then by loop theorem (see \cite{???}), there is an embedded disc in $\Omega$ with the same property --- it lies in $\Omega$,
its boundary lies in $W\cap\Omega$
and is not null-homotopic in $W\cap\Omega$.
The latter contradicts Definition~\ref{def:two-convex}.
\qeds
...
The statement might look obvious, but there is an underwater stone in the proof. %%%???please check English and remove this note
...
$W\backslash \Omega$ contains an isolated point.%%%???please check English and remove this note
\end{thm}

\parit{Proof of Lemma~\ref{lem:poly-shefel}.}
The ``only if'' part can be proved the same way as in Theorem~\ref{thm:set-with-smooth-bry:CBA} with additional use of Exercise~\ref{ex:inner-support}.%%%???please check English and remove this note
...
(It may happen only on the first step --- in the construction of $\Omega_1$ form $\Omega_0$, but we do not use it.)
