...
For    $\Cat{}{0}$ the statement follow from convexity of distance functions to points in $\EE^2$ and thinness of triangles.
For  $\Cat{}{1}$  the statement  follows from the thinness of triangles and convexity of balls of radius $<\pi/2$ in $\SS^2$.
\begin{comment}
...
\parbf{Lemma.}
\emph{Let $\spc{S}$ be a simplicial complex which 
...
Assume $\Omega$ is not two-convex.
Then, by the definition, there is a curve $\gamma$ which lies in the intesection of a plane $W_0$ and $\Omega$
which is contactable in $\Omega$ but not contractable in $\Omega\cap W_0$.

Since $\Omega$ is open,
the disc shrinking $\gamma$ 
as well as $\gamma$ can be taken in general position.
So we can assume that the disc with the plane $W_0$ cuts from the space $\EE^3$ finite number of bounded domains.

Since $\gamma$ is not contactable in $\Omega\cap W_0$,
one of these domains, say $Q$, contains a point from the complement of $\Omega$.
By general position argument we can assume that $Q$ such point lies in the interior of $Q$.

Fix circle $\Gamma$ in $W_0$ which has $Q\cap W_0$ inside.
Consider the smallest spherical dome with boundary $\Gamma$ which includes the set $R=Q\backslash\Omega$.
The dome has to touch $R$ at some point $p$;
the plane tangent to the dome at this point has the required property.
%%%???this solution is new please check and remove this note



If $\Omega$ is not two-convex then 
by definition there is a map $\phi$ form disc $\DD$ to $\Omega$, which boundary maps to a plane curve which is
