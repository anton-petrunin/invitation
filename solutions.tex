...
%???it is not a semisolution should we add more or simply give no solution here???
...
Despite that the statement has simple visual proof, it is quite hard to write it formally in a nontedious way;
...
Show that there is a line segment of $\partial \bar S$ starting at $x$ or $y$ which lies in $\Pi$.

From the latter statement and local convexity of $\bar S$, 
all points of $[pq]$ sufficiently close to $p$ lie in $\bar S$,
a contradiction.
\qeds
