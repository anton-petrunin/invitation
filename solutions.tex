...
A closely related statemet was proved by Wallace Wilson \cite[§ 12]{wilson}; see also \cite{blumenthal}.
...
\parbf{\ref{ex:angle-on-shortest}.} ???
...
Let me also recommend a video by Grant Sanderson \cite{sanderson} about a peculiar connection between $\pi$ and the number of collisions in one-dimensional billiards.
...
The majorization theorem for polygons is \ref{sec:polygons}

By the majorization theorem, $P=[x^1\dots x^{n}]$ can be majorized by a convex plane polygon, say $\dot P=[\dot x^1\dots \dot x^{n}]$.
Note that we have
\[
...
\ge
\mangle\hinge{x^i}{x^{i-1}}{x^{i+1}}
\]
for all $i$, and $\dist{\dot x^0}{\dot x^{n+1}}{\EE^2}
=
...
\dist{\dot x^i}{\dot x^{i-1}}{\EE^2}
=
\dist{\tilde x^i}{\tilde x^{i-1}}{\spc{U}}
\quad \text{and}\quad
\mangle\hinge{\dot x^i}{\dot x^{i-1}}{\dot x^{i+1}}
\ge
