...
Imagine that each ball has zero radius, then may think that balls pass thru each other.
...
To do this, one have to exclude the space occupied by the balls.
In other words, if the $i^{\text{th}}$ ball centered at $x$, then the we assume that its coordinate is 
$x-2\cdot r\cdot i$.
With these new coordinates, the balls behave exactly as the balls with vanishing radii.

\parbf{\ref{ex:collision}.}
Assume that trajectory $\gamma$ defined in an interval $[a,b)$
and collisions accumulate at $b$.
