...
and the set $\bar{A}_\infty$ of all partial (i.e. subsequential) limits of such sequences is called the \emph{upper closed limit}.
...
apply that the triangle $[p\,\gamma(t_0)\,\gamma(t_1)]_{\spc{U}}$ is thin.
 The "if" direction is proved by reversing the above argument.
...
\parit{Comment.}
This statement generalizes a theorem of Heinrich Tietze ~\cite{tietze}, and the proof presented here is nearly identical to the original proof given by Tietze.
...
Note that in the original Fáry--Milnor theorem, the inequality is strict.\footnote{\color{red} V: the last sentence is very unclear as is its relevance here. }
...
It is clearly enough to show that  $\spc{P}$ is locally $\CAT(0)$. Assume the contrary.
...
\footnote{ \color{red} I don't understand what this statement means}

In the proof, one can apply \ref{ex:geod-circle};
...
It follows that $(Q,\ell^\infty)$ is not an injective space --- it does not contain a point at distance $\tfrac14$ from $m_x$, $m_y$, and $m_z$ --- a contradiction.
...
One needs to adapt the proof of the proof of Theorem~\ref{thm:bishop-plane} to the spherical case. From  Exercise~\ref{ex:hemisphere}, it follows that if the image in $\mathbb{S}^2$  of a lune in $K$ has perimeter smaller then $2\cdot\pi$, then it contains a closed hemisphere in its interior or lies in an open hemisphere.
...
but whose interior contains points from the complement $\EE^m\setminus K$. This triangle is not thin in $K$.
\parbf{\ref{ex:two-convex-not-a-CAT}.}
Clearly, the set $W$ is two-convex, and so is $K$ as the intersection of two two-convex sets.
...
Apply Sperner's lemma \footnote{\color{red} what is Sperner's lemma? } to show that $\spx{\bm{p}}(\triangle^k)(x)\z\in \bigcap_iK_i$ for some $x\in \triangle^k$,
