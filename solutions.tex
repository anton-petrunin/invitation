...
%the Euclidean space or just Euclidean space?
...
then the corresponding projection moves continuously and the projections of 
%4 triangles 
the 4 faces 
cannot degenerate. 
...
Assume $\bm{x}(t)$ and $\bm{x}'(t)$ are two curves parametrized by $\rho_0$-length which differ only in the $m$-th coordinate, denoted by $x_m(t)$ and $x_m'(t)$ correspondingly.
...
Let $z$ be the limit of $z_n$ as $n\to\infty$. Clearly $z$ is a midpoint  between $x$ and~$y$.\qeds 
...
%a generalization of arm lemma to
%this is confusing since arm lemma is not  standard term
the arm lemma in
%
$\Cat{}{\kappa}$ spaces; see our book \cite{AKP}.
%
...
Then choosing a sequence of unit speed noncontractible curves whose lengths converge to $L$, an Arzela-Ascoli type of argument shows that these curves subconverge to a noncontractible curve of length $L$.
%I feel this is too brief and would like to expand this. perhaps add an exercise that close curves are homotopic? V.
...
Note that the triangle $\trig pqx$ is degenerate, in particular it is thin.
...
Even though the proof is simple and visual, it is quite hard to write it formally in a non-tedious way;
