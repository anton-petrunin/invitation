...
\parit{Comment.}
This solution is taken from \cite{petrunin-quest}.
...
The presented proof is not quite standard, it was suggested to the third author by Travis Morrison.
...
Apply the homotopy from \ref{ex:contractible} to prove that $\spc{U}$ has vanishing local homology groups (and/or homotopy groups)  of at $x$.
...
The unit disc in the plane with attached half-line to each point is a complete $\CAT(0)$ length space with extendable geodesics. 
...
Imagine that each ball zero radius, then may think that balls pass thru each other.
...
This statement holds if $\gamma_1$ and $\gamma_2$ run along the opposite sides of a convex quadrangle in the plane.
Applying the majorization theorem to the quadrangle $[\gamma_1(0)\,\gamma_1(1),\gamma_2(1)\,\gamma_2(0)]$, we get the general case.

\parbf{\ref{ex:convex-nbhd}.}
It is sufficient to show that 
\[\distfun_A\circ\gamma(t)\le (1-t)\cdot\distfun_A\circ\gamma(0)+t\cdot \distfun_A\circ \gamma(1).
\eqlbl{eq:distfun(gamma(t))}\]
...
Since $\eps>0$ is arbitrary, we get \ref{eq:distfun(gamma(t))}.
...
\parit{Comment.}
This is a $\CAT(0)$ version of the famous Fáry--Milnor theorem.
In the original theorem, the inequality is strict.

\parbf{\ref{ex:arm-lemma}.}
Apply the majorization theorem and the standard arm lemma.

\parbf{\ref{ex:isometric-majorization}.}
Use the majorization theorem, to reduce the question to the following.
\textit{If a plane curve bounds a convex figure $F$,
then its majorization map is distance-preserving.}

To prove the latter statement assume that the majorization map is defined on a convex figure $F'$.
Identify the boundaries $\partial F$ and $\partial F'$ by the majorization map.
Show that the curvature measure of $\partial F'$ cannot exceed the curvature measure of $\partial F$.
On the other hand the total curvatures of $\partial F'$ and $\partial F$ are equll $2\cdot\pi$.
Therefore, $\partial F'$ and $\partial F$ have the same curvature measures,
and hence the majorization map is distance-preserving.
...
Then choosing a sequence of unit speed noncontractible curves whose lengths converge to $L$, an Arzel\'{a}--Ascoli type of argument shows that these curves subconverge to a noncontractible curve $\gamma$ of length~$L$.
...
\parit{Comment.}
Note that the statement of the exercise fails if the requirement that $\spc{X}$ be compact is replaced by the assumption that it is proper.
...
Apply the globalization theorem (\ref{thm:hadamard-cartan}) with \ref{ex:cone+susp} and the previous exercise \ref{ex:flag>=pi/2}. 
...
or a product of a tripod with real line.
