...
\begin{lpic}[t(-3mm),b(-3mm),r(0mm),l(0mm)]{pics/inner-support(1)}
\lbl[t]{40,.5;$W_0$}
\lbl[t]{27,.5;$\Gamma$}
\lbl[br]{12,12;$p$}
\lbl{15,6;$R$}
...
Applying Loop Theorem (arguing as in the proof of Proposition~\ref{prop:3d-strong-2-convexity}) we can assume that $\phi$ is an embedding and $\phi(\DD)$ lies on one side from $W_0$.

Let $Q$ be the bounded domain cut from $\EE^3$ by $\phi(\DD)$ and $W_0$. 
By assumption it contains a point which is not is $\Omega$. 
...
The plane $\Pi$ tangent to the dome at $p$ has the required property.\qeds
