...
Indeed the set of vectors $v$ such that $W(v)<0$
...
Assume $\bm{x}(t)$ and $\bm{x}'(t)$ are two curves parametrized by $d_0$-length which differ only in the $m$-th coordinate, denoted as $x_m(t)$ and $x_m'(t)$ correspondingly.
...
The $\Cat{}{1}$ comparison for $\dist{p'}{q'}{}$ with $\~z'\in[\~x'\~y']_{\SS^2}$ implies the 
$\Cat{}{0}$ comparison for $\dist{p}{q}{}$ with $\~z\in[\~x\~y]_{\EE^3}$.

The converse holds as well.
In the proof we need to apply 
...
Note that the unit disc in the plane with attached half-line to each point is a complete $\Cat{}{0}$ length spaces with extendable geodesics but the space of geodesic directions on the boundary of the disc is not complete.
...
For $\Cat{}{1}$ case,  the statement  follows from  thinness of triangles and convexity of balls of radius $<\tfrac\pi2$ in~$\SS^2$.
\begin{comment}
...
Since $K$ is connected and the latter implies that $K$ is path connected.
...
Then the set  $\dot A$ ($\dot B$) is defined as the unique supporting half-space to $A$ ($B$) at $z_0$ containing $A$ ($B$).
...
Fix $0<r_1<r_2<\infty$ such that 
\[B_1=\oBall(0,r_1)\subset A^i\subset \oBall(0,r_2)=B_2\]
for each wall~$A^i$.
...
The $\Cat{}{0}$ case is done the same way, but we need to consider the homotopy classes of curves shorter than $2\cdot \pi$.\qeds
...
It might be worth mentioning that by the globalization theorem (\ref{thm:hadamard-cartan}) the obtained metric on $\mathbb{S}^3$ is not locally $\Cat{}{0}$.
...
The only difference is that a geodesic may spends time \emph{at least} $\pi$ on each visit in the star.
\qeds
...
A triangle in the link can be described by tree ways to resolve degeneracy of one edge,
such that any pair of these resolutions can be done simultaniousely, but all theree can not be done together.
...
Extend it to a flag complex $\mathcal{T}$ by gluing a simplex in every complete subgraph of its one-skeleton.
...
Note that triangle $[pqx$ is degenerate, in particular it is thin.
...
we will use Exercise~\ref{ex:hemisphere}.
...
Assume contrary, then $\alpha$ intersects the boundary  great circle in a point, say~$r$.
...
Show that there is a line segment $[pq]$ in $\EE^3$ tangent to $\bar S$ at $p$ which has no common points with $\bar S$ except~$p$.
...
Use that $[pq]$ is tangent to $S$ to show that there is a point $z$ such that the line segment $[xz]$ or $[yz]$ lies in $\partial \bar S\cap\Pi^+$.
...
The set $K$ on large scale looks like the complement of two three-dimensional half-spaces in~$\EE^4$.
...
the completion of the complement, equipped with length metric, 
 of the union of half-spaces in two three-dimensional spaces in $\EE^4$ containing the origin. 
In particular $K'$ is a cone over a space $\Sigma$ which 
can be obtained the following way: 
Taking the sphere $\SS^3$ with  two two-dimensional hemispheres, say $H_1$ and $H_2$, removed;
equip it with induced length metric and pass to the completion.
The intersection of these hemispheres is typically formed by a geodesic segment, say $[\xi\zeta]$.


\begin{wrapfigure}{r}{55mm}
\begin{lpic}[t(-8mm),b(0mm),r(0mm),l(0mm)]{pics/halph-spheres(1)}
\lbl[r]{20,12;$\xi$}
\lbl[l]{34,12;$\zeta$}
\lbl{10,12;$H_1$}
\lbl{45,12;$H_2$}
...
Consider  hemispheres $H_1$ and $H_2$ for which $[\xi\zeta]=H_1\cap H_2$ is orthogonal to the boundary circles and such that 
