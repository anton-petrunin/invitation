...
The following two corollary follow from 
(1) the patchwork (\ref{lem:patch}),
(2) Proposition \ref{cor:loc-geod-are-min}, which states that local geodesics are geodesics in any $\Cat{}{}$ spaces, 
...
Hence $\II=[0,1]$.
It remains to set $\gamma_\alpha=\gamma_1$

By construction,  if $\alpha$ is a local geodesic path, then $\gamma_\alpha=\alpha$. 

Moreover, from Corollary \ref{cor:discrete-paths},
the construction $\alpha\mapsto \gamma_\alpha$ produce close results for sufficiently close paths;
