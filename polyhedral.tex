...
to be moved continuously so that its endpoints follow given trajectories, a procedure obtained by a different method in \cite{a-b:cbc}.
...
In the globalization theorem (\ref{thm:hadamard-cartan}) properness can be weakened to completeness, see \cite{AKP} and the references there in.
...
For example the description of coverings of $\SS^3$ branching at three 
great circles which are $\Cat{}{1}$ require quite a bit of work;
see \cite{charney-davis-93} --- try to guess the answer before reading.
An other example, is the space $\spc{B}_4$ which is the universal cover of $\CC^4$ infinitely branching in the complex planes $z_i=z_j$ with the induced length metric.
So far it is not known if $\spc{B}_4$ is $\Cat{}{0}$ 
despite that this space could be helpful to study the braid group with 4 strings;
read \cite{panov-petrunin:ramification} for more on it.
This circle of questions is closely related to the generaliztion of the flag condition (\ref{thm:flag}) to the spherical simplices with few acute dihedral angles.
