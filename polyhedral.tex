...
and $\gamma\:[0,1]\to\spc{U}$ be a local geodesic path.
...
Note that if $\tilde \gamma$ is a geodesic with the same endpoints as $\hat\gamma$ then $\Phi\circ \tilde \gamma$ is a local geodesic with the same endpoints as $\gamma$.
...
and Theorem \ref{thm:cat-unique} on uniqueness of geodesics~.
...
there is a choice of a local geodesic path $\gamma_\alpha$ with length less than $\pi$ connecting the ends of $\alpha$ such that the map $\alpha\mapsto\gamma_\alpha$ is continuous, and if $\alpha$ is a local geodesic path then $\gamma_\alpha=\alpha$.
\end{thm}

\parit{Proof of Corollary \ref{cor:path-geod}; $\Cat{}{0}$ case.} 
We argue using an open-closed argument. Let $S\subset[0,1]$ be the set of such $t$ so that we can construct a continuous  family of local geodesic paths $\gamma_{\alpha}^s :[0,s]\to \spc{U}$ for all $0\le s\le t$, where the length of $\gamma_{\alpha}^s$ is at most that of  $\alpha|_{[0,s]}$ for all $0\le s \le t$. Since a limit of local geodesics is a local geodesic by Proposition~\ref{prop:geo-complete}, $S$ is closed.

Applying Patchwork \ref{lem:patch} to  $\gamma_{\alpha}^t$ for $t\in S$ it's easy to see that $S$ is also open in $[0,1]$. Hence $S=[0,1]$.
Continuity of the construction follows from Patchwork.

 Moreover, by construction,  if $\alpha$ is a local geodesic path, then $\gamma_\alpha=\alpha$. 
...
and by Stephanie Alexander and Richard Bishop in \cite{a-b:h-c} in more generality.
...
%Corollary~\ref{cor:closed-geod-cat}
...
and all the other vertices of degree 3, 
...
Let $\spc{S}$ be a simplicial complex and $\{v_1,\dots,v_N\}$ be the set of its vertices.
...
Since both $V_v$ and $W$ are contractible, the identity map of  their common boundary $\Sigma$ can be extended to a homotopy equivalence relative to the boundary between $V_v$ and $W$.
