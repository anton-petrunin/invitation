...
and hence is  minimizing by Corollary~\ref{cor:loc-geod-are-min}.  
...
If $\spc{U}$ is a locally $\Cat{}{0}$ proper length space, then for any pair of points $p,q\in\spc{U}$, the space of all locally geodesic paths from $p$ to $q$ is discrete;
...
Analogously, if $\spc{U}$ is a locally $\Cat{}{1}$ proper length space, then for any pair of points $p,q\in\spc{U}$,  the space of all locally geodesic paths from $p$ to $q$ with length less than $\pi$ is discrete.
\end{thm}
...
Assume $\spc{U}$ is a locally $\Cat{}{1}$ proper length space and consider the construction $\alpha\mapsto\gamma_\alpha$ provided by Corollary \ref{cor:path-geod}.
...
The original formulation of the \emph{globalization theorem}, or \emph{Hadamard--Cartan theorem} states that if $M$ is a complete Riemannian manifold with sectional curvature $\le 0$,  
...
In this generality, this theorem appeared in the lectures of Cartan, see \cite{cartan}.
This theorem was proved for surfaces in  Euclidean $3$-space 
 by
Hans von Mangoldt \cite{mangoldt},  
and a few years later independently for $2$-dimensional Riemannian manifolds by Hadamard \cite{hadamard}.
...
A detailed proof of Gromov's statement when $\spc{U}$ is proper  was given by Ballmann in \cite{ballmann:cartan-hadamard}
and generalized further in \cite{a-b:h-c}.  

For  $\Cat{}{1}$ proper spaces, the globalization theorem was proved by Bowditch in \cite{bowditch}.
...
Any compact locally $\Cat{}{0}$ space which contains no closed geodesics is $\Cat{}{0}$.
...
which must be a closed geodesic. 
...
is defined as the maximal dimension of the simplexes 
...
for any two points $p,q\in\gamma$, one of the arcs of $\gamma$ with endpoints $p$ and $q$ forms a minimizing geodesic. 
\end{thm}
...
The $\Cat{}{0}$ case is exactly the same  except we need to use the first part of Exercise~\ref{ex:cone+susp} and the the first part of Corlollary~\ref{cor:closed-geod-cat}.
\qeds
...
The product of the geodesics $[v^1v^2]$, $[v^2v^3]$ and $[v^3v^1]$
...
{\color{red}!!! Should this equation be clarified somewhere in the preliminaries?  there is no section on modified distance functions!!!}
...
with simplices {\color{red} !!! it's better use "simplices" everywhere or "simplexes" everywhere. currently it's inconsistent!!!} replaced by cubes.
...
The following example was proved by Michael Davis in \cite{davis-noneuclidean}.
...
{\color{red} Indeed, using contractibility  of $\spc{W}$ it's easy to construct a map  $f\: \spc Q\to \spc M$ which extends the identity map of $\spc{N}$ and maps each small neighbourhood of a vertex in $\spc{Q}$ to the corresponding copy of 
$\spc{W}$. It's easy to see that this map induces an isomorphism on $\pi_1$ and its lift to the universal covers $\tilde f \: \tilde{\spc{Q}}\to \tilde{\spc{M}}$ induces an isomorphism on homology. This means that $\tilde f$ (and hence $f$) induces an isomorphism on all homotopy groups and therefore is a homotopy equivalence.
}
...
The latter follows since homotopy equivalences 
...
that is, for any compact sets $A\subset \tilde{\spc{Q}}$ and $B\subset\tilde{\spc{M}}$ the invese images $\tilde g^{-1}(A)$ and $\tilde f^{-1}(b)$ are compact.
\qeds


\section{Comments}

The construction used in the proof of  Proposition~\ref{prop:aspherical} does many other interesting things,  
...
The construction described in this chapter also leads to so-called \emph{hyperbolization}, a flexible tool to construct a aspherical spaces;
...
So called \emph{hyperbolic groups} introduced by Mikail Gromov
