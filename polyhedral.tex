...
Since $\spc{B}=\cBall[\gamma_\infty(t),R]$ is a proper length $\Cat{}{0}$ space, then by Theorem~\ref{thm:cat-unique},
...
Recall that a triangulation of $m$-dimensional manifold defines a piecewise linear structure if the link  of every simplex $\Delta$ is homeomorphic to the sphere of dimension $m-1-\dim\Delta$.
...
cannot be made piecewise linear --- despite that $\spc{P}$ is a manifold, its triangulation does not induce a piecewise linear structure.
