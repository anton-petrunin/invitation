...
The proof goes along the same lines as Exercise \ref{ex:convex-balls}.%%%???please check my English and remove this comment.
...
Namely we assume that $\spc{U}$ is a $\Cat{}{1}$ proper length space and construct a $\Cat{}{1}$ proper length space $\spc{N}$ with property as above.
\end{thm}
...
\parit{Proof of Corollary \ref{cor:path-geod}.} 
...
local geodesic paths $\gamma_t$ connecting $\alpha(0)$ to $\alpha(t)$ for any $t\in\II$ and $\gamma_0(\tau)=\alpha(0)$ for any $\tau$. 
...
%???please check my English and remove this comment.
\end{thm}
...
There are no 3-dimensional examples of that type, see \cite{rolfsen} by Dale Rolfsen.
In \cite{thurston}, Paul Thurston conjectured that the same holds in 4-dimensional case.%%%???please check my English and remove this comment.
...
An other example of that type was constructed by Stephan Stadler, see \cite{stadler}.
...
Moreover, according to Stone's theorem, see \cite{stone, davis-januszkiewicz}, there is no piecewise linear analog of the proposition; %???Check ref???
