...
In this chapter we introduce locally $\Cat{}{0}$ spaces and prove the globalization theorem that provides a sufficient condition for locally $\Cat{}{0}$ spaces to be globally $\Cat{}{0}$.

The theorem implies in particular that the universal metric cover of a proper 
length, locally $\Cat{}{0}$ space is a proper length $\Cat{}{0}$ space. 
...
which is essentially  \cite[Satz 9]{alexandrov:devel}.  
...
Note that the implication ``only if'' is already proved in  Theorem~\ref{thm:cat-unique}; it only remains to prove the ``if'' part.
...
Note that $\ell>0$;
otherwise there would be a closed noncontractible curve in a $\Cat{}{0}$ neighborhood of some point, contradicting Corollary \ref{cor:contractible-cat}.
...
such that every simplex in $\tau$ is isometric to a simplex in a Euclidean space (respectively a unit sphere) of appropriate dimension.
...
%Should this be plural like in section headings? V.
...
that are pairwise joined by edges, then the vertices $v^0,\dots,v^\kay$
...
Louis Billera,
Susan Holmes,
and  Karen Vogtmann in~\cite{BHV}.
...
By \ref{cor:contractible-cat}, any proper length $\Cat{}{0}$ space is contractible.
...
The latter can only fail if $\gamma$ contains two vertices, say $\xi$ 
and 
$\zeta$ in $\spc{S}$,
which are joined by an edge not in $\gamma$; 
denote this edge by~$e$.

Indeed, if $G$ is \emph{not} locally convex in~$\spc{Q}$, then $\gamma$ is \emph{not} $\pi$-convex in $\spc{S}$; that is, there are two points $\eta$ 
and 
$\theta$ such that $\dist{\eta}{\theta}{\spc{S}}<\pi$, but the geodesic $[\eta\theta]_{\spc{S}}$ is not an arc of $\gamma$.
We may also assume that $\eta$ or $\theta$ is not a vertex of $\spc{S}$; otherwise $\dist{\eta}{\theta}{\spc{S}}<\pi$ implies $\dist{\eta}{\theta}{\spc{S}}=\tfrac\pi2$, and the problem is solved.
Without loss of generality assume $\eta$ is not a vertex,
so it lies inside an edge $[\xi\xi']$ of $\gamma$.
Further we can assume that only the endpoint of $[\eta\theta]_{\spc{S}}$ lie on $\gamma$.
Leaving $\eta$ the geodesic $[\eta\theta]_{\spc{S}}$ enters a simplex $\Delta$;
choose a vertex $\zeta$ in $\Delta$ distinct from $\xi$ and $\xi'$.
Since geodesic $[\eta\theta]_{\spc{S}}$ is shorter than $\pi$, the same argument as in \ref{thm:flag} shows that 
$[\eta\theta]_{\spc{S}}$ spends the rest of its life in the open star $\Star_\xi$.
In particular the other end $\theta$ lies on an edge from $\xi$.
It follows that $\xi$ lies on $\gamma$ and we can take $e$ to be $[\xi\zeta]$ or $[\xi\zeta']$.
...
The later means that any short map $f\:A\to (Q,\ell^\infty)$ defined on a subset $A$ of an arbitrary metric space $\spc{X}$ admits a short extension to whole space $\spc{X}$. 
\end{subthm}
...
This statement can be extracted from a paper of Brian Bowditch \cite{bowditch-2020}; two easy parts of the theorem are included in the following exercise.
