...
It comes with a triangulation of special type; namely the link  of every simplex $\Delta$ is homeomorphic to the sphere of dimension $m-1-\dim\Delta$.
Manifolds with such triangulations are called \index{piecewise linear manifold}\emph{piecewise linear}.
 
It noteworthy that some manifolds admit triangulations which do not describe piecewise linear manifolds.
For example the sphere $\SS^5$ is homeomorphic to the double suspension over the Poincare sphere $\Sigma^3$.
A triangulation of $\Sigma^3$ induces a triangulation of $\SS^5$ in which some 1-dimensional simplices have $\Sigma^3$ as link.
