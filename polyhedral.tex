...
Any compact, locally $\Cat{}{0}$ space that contains no closed local geodesics is $\Cat{}{0}$. %no need to be length space
...
Prove that any compact length, locally $\Cat{}{0}$ space that is not $\Cat{}{0}$ contains a \index{geodesic circle}\emph{geodesic circle}, 
...
For that reason a flag complex also called a \index{clique complex}\emph{clique complex}.
...
It admits a natural all-right triangulation --- each simplex corresponds to an adjusted cube.
...
The following proposition provides an example of exotic aspherical manifold of dimension $m\ge4$.
It comes with a triangulation of special type; namely link  of every simplex $\Delta$ is homeomorphic to the sphere of dimension $m-1-\dim\Delta$.
Manifolds with such triangulation are called \index{piecewise linear manifold}\emph{piecewise linear}.
 
It noteworthy that some manifolds admit triangulations which do not describe a piecewise linear manifold.
For example the sphere $\SS^5$ is homeomorphic to the double suspension over Poincare sphere $\Sigma^3$.
A triangulation of $\Sigma^3$ induces a triangulation of $\SS^5$ in which some 1-dimensional simplices have $\Sigma^3$ as its link.
Therefore, with this triangulation, $\SS^5$ is not piecewise linear.
Manifolds with arbitrary triangulations are considered in Proposition~\ref{prop:loc-CAT-mnfld}
%%%???please check last two pars; maybe it is a good idea to remove all PL-parts from the proposition and move it to the comments???
...
In particular, 
$C_R$ is not contractible in $\tilde{\spc{Q}}\backslash K$
if $K\supset \tilde v$.
If $R$ is large, 
the circle $C_R$  
...
They constructed a closed smooth four-dimensional manifold $M$ with the universal cover $\tilde M$ diffeomorphic to $\RR^4$, such that $M$ admits a polyhedral metric which is locally $\Cat{}{0}$, but does not admit a Riemannian metric with nonpositive sectional curvature.
...
cannot be made piecewise linear.
