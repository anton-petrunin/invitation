...
In this chapter we introduce locally $\Cat{}{0}$ spaces and show that the universal cover of a locally $\Cat{}{0}$ proper length space forms a $\Cat{}{0}$ proper length space. 
...
In this section we will study behavior of local geodesics in locally $\Cat{}{}$  spaces.  
...
so that its endpoints follow given trajectories, a construction from \cite{a-b:cbc}.
...
(\ref{lem:patch}), Proposition \ref{cor:loc-geod-are-min} ($\Cat{}{}$ local geodesics are geodesics), 
...
The \emph{product} path obtained by concatenating two paths will be denoted with ``$*$'';
...
and few years later independently for $2$-dimensional manifolds by Hadamard \cite{hadamard}.
...
The surface of revolution showed on the diagram, 
is an example of simply connected space which is locally $\Cat{}{1}$ but not $\Cat{}{1}$.
...
The proof use a thin-triangle decompositions with the inheritance lemma (\ref{lem:inherit-angle}) and the following construction:
...
By the assumptions the line-of-sight map (\ref{def:sight}) $(t,\tau)\mapsto\gamma_t(\tau)$ for  $[x y]$ from $p$ is uniquely defied and continuous.    
...
but one need to use Exercise~\ref{ex:null-homotopic}. \qeds
...
Note that $\ell>0$;
otherwise there would be a closed noncontractable curve in a $\Cat{}{0}$ spherical neighborhood of some poin which contradicts Corollary \ref{cor:contractible-cat}.
...
In the assumptions of Corollary~\ref{cor:closed-geod-cat},
prove that the space contains \emph{geodesic circle}; 
that is a simple closed curve $\gamma$ such that 
for any two points $p,q\in\gamma$ one the arcs of with endpoints $p$ and $q$ forms a minimizing geodesic. 
\end{thm}
...
Assume in a polyhedral space $\spc{P}$
