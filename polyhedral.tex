...
Fix $t\in[0,1]$.  
Let $R>0$ be small enough so that $\cBall[\gamma_\infty(t),R]$ 
...
each cube of $\spc{Q}$ is isometric to the unit cube in $\RR^k$ with $k\le N$.
%the same dimension.
%Why should all the cubes have the same dimension?? V.
...
if $K\supset \tilde v$ and $C_R$ is contained in $\tilde{\spc{Q}}\backslash K$.
If $R$ is large, 
the circle $C_R$  
