...
We say that space $\spc{U}$ is locally $\Cat{}{0}$ (or locally $\Cat{}{1}$) if
a small closed ball centered at any point $p$ in $\spc{U}$ forms a $\Cat{}{0}$ (or correspondingly $\Cat{}{1}$ space).
These definition are equivalent to saying that each point $p$ admits an open neighborhood $\Omega$
...
In this chapter we will prove globalization theorem for $\Cat{}{}$ spaces;
that is we give some conditions for locally $\Cat{}{}$ which guarantee that they are globally $\Cat{}{}$.
The globalization theorem leads to a \emph{construction toy set}, described by Flag condition~\ref{thm:flag}.
This toy set can be used to produce examples of exotic aspherical spaces.
...
The proved statements will be used in the proof of the Hadamard--Cartan theorem (\ref{thm:hadamard-cartan}).

\begin{thm}{Proposition}\label{prop:geo-complete}
Let $\spc{U}$ be a locally $\Cat{}{0}$ or locally $\Cat{}{1}$ space.
...
Since $\cBall[\gamma_\infty(t),R]$ is a $\Cat{}{0}$ space, by Theorem~\ref{thm:cat-unique},
...
Then there is a $\Cat{}{\kappa}$ space $\spc{N}$,
...
\parit{Proof.} 
Fix $r>0$ so that the closed ball
$\cBall[\gamma(t),r]$ forms a $\Cat{}{\kappa}$ space.
...
we conclude that $\spc{N}$ is a $\Cat{}{\kappa}$ space.
...
\begin{thm}{Corollary}
If $\spc{U}$ is a locally $\Cat{}{0}$ space than for any pair of points $p,q\in\spc{U}$  the space of all locally geodesic paths from $p$ to $q$ is discrete;
...
Analogously, if $\spc{U}$ is a locally $\Cat{}{1}$ space than for any pair of points $p,q\in\spc{U}$  the space of all locally geodesic paths from $p$ to $q$ with length less than $\pi$ is discrete.
\end{thm}

The spaces of paths $\alpha\:[0,1]\to\spc{U}$ also comes with the natural metric
\[\dist{\gamma_1}{\gamma_2}{}=\sup\set{\dist{\gamma_1(t)}{\gamma_2(t)}{\spc{U}}}{t\in[0,1]}.\]

\begin{thm}{Corollary}\label{cor:path-geod}
Let $\spc{U}$ is a locally $\Cat{}{0}$ space than 
...
Analogously, if $\spc{U}$ is a locally $\Cat{}{1}$ space than 
...
For $\Cat{}{1}$ spaces Globalization theorem was proved by Bowditch in \cite{bowditch}.

\begin{thm}{Globalization theorem}
\label{thm:hadamard-cartan}
Any simply connected locally $\Cat{}{0}$ space 
is $\Cat{}{0}$.

Analogously, assume $\spc{U}$ is a locally $\Cat{}{1}$ space
...
A locally $\Cat{}{0}$ space $\spc{U}$ is $\Cat{}{0}$
...
Analogously, a locally $\Cat{}{1}$ space $\spc{U}$ is $\Cat{}{1}$ 
...
but one has to be more careful with the condition on the paths $\alpha$ and $\beta$ which guarantee $\gamma_\alpha=\gamma_\beta$.
...
Assume $\spc{U}$ is a locally $\Cat{}{1}$ space.
Given two paths $\alpha$ and $\beta$ in $\spc{U}$ with lengths less than $\pi$,
...
The proof of the claim is straightforward and left to the reader.
\qeds

\section{Polyhedral spaces}

\begin{thm}{Definition}\label{def:poly}
A space $\spc{P}$ is called  
...
Analogousely, a space $\spc{P}$ is called  
...
Analogously, if $\spc{P}$ be a spherical polyhedral space and $\tau$ be a its triangulation. Then $\spc{P}$ is $\Cat{}{1}$ if and only if $\spc{P}$ and every link of any simplex in $\tau$ has no closed local geodesic shorter than $2\cdot\pi$.
\end{thm}
...
Conclude that any simplicial complex is homeomorphic to a $\Cat{}{1}$ space.
\end{thm}
...
then $\spc{Q}$ is a locally $\Cat{}{0}$ space 
...
any $\Cat{}{0}$ space is contractible.
Therefore any complete locally $\Cat{}{0}$ space 
is \emph{aspherical};
...
Is it true that $\spc{B}_n$ is a $\Cat{}{0}$ space for any $n$.
\end{thm}
...
??? is a good survey in the subject.
