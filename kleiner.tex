...
that is, $\bm{m}=(m_0,\dots,m_k)\in\triangle^k$ if $m_0+\dots+m_k=1$ and $m_i\ge0$ for all $i$.

Consider a point array $\bm{p}=(p_0,\dots,p_k)$ in the Euclidean space $\EE^n$.
Recall that
\[z=m_0\cdot p_0+\dots+m_k\cdot p_k\]
is called the  \index{barycenter}\emph{barycenter} of the point array $\bm{p}=(p_0,\dots,p_k)$ with masses $\bm{m}\z=(m_0,\dots, m_k)\in \triangle^k$.
Equivalently, 
\[z\df \argmin (m_0\cdot f_0+\dots+m_k\cdot f_k),\eqlbl{eq:f-spx}\]
where $f_i=\tfrac12\cdot\distfun_{p_i}^2$ for each $i$, and $\argmin f$\index{$\argmin$} denotes a point of minimum of the function $f$.

The map $\spx{}\:\triangle^k\mapsto \EE^n$ defined by $\spx{}\:\bm{m}\mapsto z$ is called the \index{barycentric simplex} \emph{barycentric simplex} of the array $\bm{p}$.
...
although, the map might be undefined or non-uniquely defined.
...
This will be the only way we'll use the  $\CAT(0)$ condition for most of this chapter.
...
$\spx{\bm{f}}\:\triangle^k\to \spc{X}$ is a uniquely defined Lipshitz map.

In particular, we have that for any point array $\bm{p}\z=(p_0,\z\dots,p_k)$ in a complete geodesic $\CAT(0)$ space
...
\parit{Uniqueness.}
Assume that $x$ and $y$ are distinct minimum points of $f$. Let $z$ be a midpoint of the geodesic $[x y]$.
...
It follows that given $\bm{x}\in\triangle^k$, there is a constant $\Lip$ such that
\begin{align*}
...
Note that since $\triangle^k$ is compact, we also get that $W$ is closed.
...
Note that if $\bm{f}(p)\succcurlyeq\bm{w}$ for some $\bm{w}\in W$, then  $\bm{f}(p)\succcurlyeq\bm{w}\succcurlyeq\bm{f}(x)$ for some $x\in \spc{X}$ and hence $x=p$ by the uniqueness of $\spx{\bm{f}}$. Therefore  $\bm{f}(p)=\bm{w}$.
...
for some $\bm{m}=(m_0,\dots,m_k)\in\triangle^{\kay}$.
Let $p=\spx{\bm f}(\bm m)$.
...
$\bm{f}(p)$ is supported by a plane as above for some $\bm{m}\in\triangle^{\kay}$,
but it is not supported by a plane for some $\bm{m}\in\partial\triangle^{\kay}$.
...
coincides with the restriction of $\spx{\bm{f}}$ to the corresponding facet of $\triangle^k$.

If $\Im \spx{\bm{f}}$ is not covered by the union of $\Im \spx{\bm{f}^{\without i}}$ over all $i$
...
The invariants satisfying the following two statements \ref{dim-axiom-norm} and \ref{dim-axiom-sigma} are commonly called  the``dimension'';
...
and let $\alpha_0,\alpha_1,\dots,\alpha_n$ be the vertexes of $\triangle$.
%Denote by $\triangle$ the minimal simplex that contains $\map(x)$;
%and let $(\alpha_0,\alpha_1,\dots,\alpha_n)$ be the vertices of $\triangle$.
...
Except for Szpilrajn's theorem, there are no other relations between topological and Husdorff dimension for general separable spaces.
...
For any $\alpha\ge 1$ construct a metric on the binary tree such that it has compact completion of  Hausdorff dimension $\alpha$.
...
He also proved the dimension theorem; an improvement was proved by Alexander Lytchak \cite{lytchak:diff}.

It is not known if the dimension theorem holds for arbitrary complete geodesic $\CAT(\kappa)$ spaces.
It was conjectured that it is so by Bruce Kleiner \cite{kleiner}, see also \cite[p.~133]{gromov:asymt-inv}.
...
this seems to be unknown even for compact spaces.
...
For $n=2$, one can get it from the the fact that minimal (or metric minimizing) surfaces in $\spc{U}$ are $\CAT(0)$ \footnote{ \color{red} V: reference?}(any such surface is locally bi-Lipschitz to the Euclidean plane).
