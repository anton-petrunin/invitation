...
is called the \index{barycenter}\emph{barycenter} of the point array $\bm{p}=(p^0,\ldots,p^k)$ with masses \index{$\triangle^k$}$\bm{\mu}\z=(\mu^0,\ldots, \mu^k)\in \triangle^k$.
Equivalently, 
...
\[\Pi=\set{(x_0,\ldots,x_k)\in \RR^{k+1}}{\mu^0\cdot x_0+\ldots+\mu^k\cdot x_k=\const}\]
...
but not supported by any such plane with $\bm{\mu}\in\partial\triangle^{\kay}$.
...
Suppose that $\TopDim \spc{U}\le m$ for some $m<n$ and any $m+1$ subsets from $\{K^1,\ldots, K^n\}$ have a common point.
