...
Let $\spc{X}$ be a complete geodesic space 
and $\bm{f}\z=(f_0,\z\ldots,f_\kay)\:\spc{X}\to\RR^{\kay+1}$
be an array of non-negative 1-convex locally Lipschitz functions.
...
for some $\bm{\mu}=(\mu_0,\ldots,\mu_k)\in\triangle^{\kay}$.
...
Let $\bm{p}\z=(p_0,\ldots, p_\kay)$ be a point array
in a complete length $\CAT(0)$ space $\spc{U}$, 
and $B_i=\cBall[p_i,r_i]$ for some array of positive reals $(r_0,r_1,\ldots,r_\kay)$.
...
Show that the condition in \ref{SHORT.lem:nondeg-test-with-balls:nondeg} holds for some
 array of positive reals $(r_0,\ldots,r_\kay)$.
\end{subthm}
