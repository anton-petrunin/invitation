...
Let us denote by $\triangle^k\subset \RR^{k+1}$\index{$\triangle^m$} 
the \index{standard simplex}\emph{standard $k$-simplex}; 
that is, $\bm{\mu}=(\mu^0,\ldots,\mu^k)\in\triangle^k$ if $\mu^0+\ldots+\mu^k=1$ and $\mu^i\ge0$ for all $i$.

Consider a point array $\bm{p}=(p^0,\ldots,p^k)$ in the Euclidean space $\EE^n$.
Recall that
\[z=\mu^0\cdot p^0+\ldots+\mu^k\cdot p^k\]
is called the \index{barycenter}\emph{barycenter} of the point array $\bm{p}=(p^0,\ldots,p^k)$ with masses $\bm{\mu}\z=(\mu^0,\ldots, \mu^k)\in \triangle^k$.
Equivalently, 
\[z\df \argmin (\mu^0\cdot f^0+\ldots+\mu^k\cdot f^k),\eqlbl{eq:f-spx}\]
where $f^i=\tfrac12\cdot\distfun_{p^i}^2$ for each $i$, and $\argmin f$\index{$\argmin$} denotes a point of minimum of the function $f$.
...
The latter means that we define the map via \ref{eq:f-spx} for an array of functions $\bm{f}=(f^0,\ldots,f^k)$.
...
and $\bm{f}\z=(f^0,\z\ldots,f^k)\:\spc{X}\to\RR^{k+1}$
...
In particular, we have that for any point array $\bm{p}\z=(p^0,\z\ldots,p^k)$ in a complete geodesic $\CAT(0)$ space
...
For $t_0\in\II$, let us define the \index{right derivative}\emph{right} (respectively, \index{left derivative}\emph{left}) \index{$\phi^\pm$}\emph{derivative} $\phi^+(t_0)$ ($\phi^-(t_0)$) at $t_0$ by
...
Since each $f^i$ is $1$-convex, for any $\bm{\lambda}=(\lambda^0,\ldots,\lambda^k)\z\in\triangle^k$
the convex combination 
\[\left(\sum_i \lambda_i\cdot f^i\right)\:\spc{X}\to\RR\]
is also $1$-convex.
...
where $\|\bm{\lambda}-\bm{\mu}\|_1=\sum_{i=0}^k|\lambda^i-\mu^i|$.
...
The following exercise gives useful versions of the Jensen inequality.

\begin{thm}{Advanced exercise}\label{ex:bary-jensen}
Let $z$ be the barycenter of points $p^0,\ldots,p^k$ equipped with masses $\mu^0,\ldots,\mu^k$; we assume that $\mu^i\ge0$ for all $i$,
$\mu^0+\ldots+\mu^k=1$, and the points $p^0,\ldots,p^k$ are in a complete geodesic $\CAT(0)$ space $\spc{U}$.

\begin{subthm}{ex:bary-jensen:moment}
Show that
\[\sum_i \mu^i\cdot \dist[2]{q}{p^i}{}\ge \dist[2]{z}{q}{} +\sum_i \mu^i\cdot \dist[2]{z}{p^i}{}\]
for any $q\in \spc{U}$.
\end{subthm}
...
\begin{subthm}{ex:bary-jensen:moment+}
 Show that
\[\sum_i
\mu^i\cdot \dist[2]{z}{p^i}{}
\le
\sum_{i<j}
\mu^i\cdot\mu^j\cdot \dist[2]{p^i}{p^j}{}.\]
\end{subthm}


\begin{subthm}{ex:bary-jensen:jensen}
Try to show that
\[h(z)\le \mu^0\cdot h(p^0)+\ldots+\mu^k\cdot h(p^k)\]
...
$\bm{v}=(v_0,\z\ldots,v_\kay)$
and 
$\bm{w}=(w_0,\z\ldots,w_\kay)$,
we will write
...
Let $\spc{X}$ be a complete geodesic space 
and $\bm{f}\z=(f^0,\z\ldots,f^\kay)\:\spc{X}\to\RR^{\kay+1}$
be an array of nonnegative 1-convex locally Lipschitz functions.
...
\[\Pi=\set{(x_1,\ldots,x_k)\in \RR^k}{\mu^0\cdot x_0+\ldots+\mu^k\cdot x_k=\const}\]
for some $\bm{\mu}=(\mu^0,\ldots,\mu^k)\in\triangle^{\kay}$.
...
$\bm{f}(p)$ is supported by a plane as above for some $\bm{\mu}\in\triangle^{\kay}$,
but it is not supported by a plane for some $\bm{\mu}\in\partial\triangle^{\kay}$.
...
Show that the image of the 1-dimensional barycentric simplex for a pair of points $p^0,p^1\in\spc{U}$ is the geodesic $[p^0\,p^1]$.

Construct a $\CAT(0)$ space with a three-point array $(p^0,p^1,p^2)$ such that its barycentric simplex is nondegenerate and noninjective.
\end{thm}

\begin{thm}[!]{Exercise}\label{lem:nondeg-test-with-balls}
Let $\bm{p}\z=(p^0,\ldots, p^\kay)$ be a point array
in a complete length $\CAT(0)$ space $\spc{U}$, 
and $B^i=\cBall[p^i,r^i]$ for some array of positive reals $(r^0,r^1,\ldots,r^\kay)$.

\begin{subthm}{lem:nondeg-test-with-ball:U}
Suppose $\bigcap_i B^i\ne \emptyset$.
Show that 
\[\Im\spx{\bm{p}}\subset \bigcup_i B^i.\]
\end{subthm}

\begin{subthm}{lem:nondeg-test-with-balls:nondeg}
Suppose $\bigcap_i B^i= \emptyset$, but $\bigcap_{i\ne j} B^i\ne \emptyset$ for any $j$.
Show that 
...
Show that the condition in \ref{SHORT.lem:nondeg-test-with-balls:nondeg} holds for some
 array of positive reals $(r^0,\ldots,r^\kay)$.
\end{subthm}
...
Given $\bm{v}=(v_0,\ldots,v_\kay)\in\Pi$, consider the function
$h_{\bm{v}}\: \spc{X}\to \RR$ defined by
\[h_{\bm{v}}(p)=\max_i\{f^i(p)-v_i\}.\]
...
For some $z\in \spc{U}$ there is an array of $m+1$ balls $B^i\z=\oBall(a^i,r^i)$
such that 
...
Then for some $\kay>m$,
there is a $\kay$-simplex $\triangle^\kay$ in $\spc{N}$
such that the barycentric simplex $\sigma=\Psi|_{\triangle^\kay}$ is nondegenerate; 
that is, 
$$W=\Psi(\triangle^\kay)\setminus\Psi(\partial\triangle^\kay)\ne \emptyset.
$$
...
the image of $F\circ\Psi\circ\map$ lies in the $F$-image of the $(m-1)$-skeleton of $\spc{N}$;
...
Let $K^1,\ldots,K^n$ be closed convex subsets in a proper length $\CAT(0)$ space $\spc{U}$.
Suppose that $\TopDim \spc{U}=m$ and any $m+1$ subsets from $\{K^1,\ldots, K^n\}$ have a common point.
Show that all subsets $K^1,\ldots,K^n$ have a common point.
\end{thm}
...
The dimension theorem is the first fruitful application of barycenters to $\CAT(\kappa)$ spaces.
It is due to Bruce Kleiner \cite{kleiner};
a more exact version was proved by Alexander Lytchak \cite{lytchak:diff}.
Kleiner conjectured that the dimension theorem holds for arbitrary complete geodesic $\CAT(\kappa)$ spaces \cite{kleiner}; see also \cite[p.~133]{gromov:asymt-inv}.
