...
changing $\angk p{\bar x}{\bar y}_{\EE^2}$ to $\angk p{\bar x}{\bar y}_{\mathbb{S}^2}$ or  $\angk p{\bar x}{\bar y}_{\HH^2}$ in
in \ref{eq:def-angle} makes no difference.
...
Indeed, for any $\distfun_{\{(0,n)\}}(x)\to \infty=\distfun_\emptyset(x)$ for any~$x$.
...
Note that for each $n$, the function $\distfun_{A_n}\:\spc{X}\to\RR$ is 1-Lipschitz and nonnegative.
Therefore the sequence $\distfun_{A_n}$ converges pointwise to a 1-Lipschitz nonnegative function $f\:\spc{X}\to\RR$.
