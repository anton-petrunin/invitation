...
We suggest to skip this chapter and use it as a quick reference while reading the rest.
To learn the background in metric geometry, reader advised to read the book of Burgos and Ivanov \cite{BBI}.
...
is called \emph{distance function}\index{distance function} from $x$. 
...
The points in $\spc{V}$ formed by the equivalence class of $0\times\spc{U}$ and $\pi\times\spc{U}$ are called \emph{north} \emph{south pole} of the suspension.
...
We say that geodesic $\gamma\:\II\to \spc{X}$ from point $p$ to point $q$ if 
$\II=[a,b]$ and $p=\gamma(a)$, $q=\gamma(b)$. 
In this case the image of $\gamma$ is denoted by $[p q]$\index{$[{*}{*}]$} and with a notation abuse we also calle it \emph{geodesic}\index{geodesic}.
Given $[pq]$ we can parametrize it by distance to $p$;
this parametrization will be denoted as $\geod_{[p q]}(t)$.
Clearly $\geod_{[p q]}(t)$ is a geodesic in the sence of the first definition.
...
In general, a geodesic between $p$ and $q$ need not exist and if it exists, it need not to be unique.  However,  once we write $[p q]$ we mean that we made a choice of geodesic.

A metric space is called \emph{geodesic}\index{geodesic} if any pair of its points can be jointed by a geodesic. 

A constant-speed reparametirization of geodesic by $[0,1]$ is called a \emph{geodesic path}\index{geodesic path}.
...
A curve $\gamma\:\II\to \spc{X}$  is called a \emph{local geodesic}\index{geodesic!local geodesic}, if for any $t\in\II$ there is a neighborhood $U$ of $t$ in $\II$ such that the restriction $\gamma|_U$ is a constant-speed geodesic.  If $\II=[0,1]$, then $\gamma$ is called a \emph{local geodesic path}.
...
Directly from the definition, it follows that if a path $\alpha$ connecting two points $x$ and $y$ then 
\[\length\alpha\ge \dist{x}{y}{}.\]

If for any $\eps>0$ and any pair of points $x$ and $y$ in the metric space $\spc{X}$ there is a path $\alpha$ connecting\footnote{That is, such that $\alpha(0)=x$ and $\alpha(1)=y$.} $x$ to $y$ such that
\[\length\alpha< \dist{x}{y}{}+\eps\]
then $\spc{X}$ is called length space.
...
We may use notation $\angk p q r$ if it is evident what is the model space $\HH^2$, $\EE^2$ or $\SS^2$.
...
Then 
the following expressions for Euclidean model angles have the same sign:
...
Assume $\spc{X}$ be a metric space with defined angles for all hinges.
...
that is the angle satisfies all the conditions of metric except the angle between distinct geodesics might vanish.
...
The elements of $\T_p$ will be called \index{tangent vector}\emph{tangent vector} at $p$,
despite that $\T_p$ is only cone --- not a vector space.
The elements of $\T'_p$ will be called \index{geodesic tangent vector}\emph{geodesic tangent vector} at $p$.
