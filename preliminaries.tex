...
However,  once we write $[p q]$ we mean that we made a choice of a geodesic.
...
However, if we write $\trig p q r$, it means that we made a choice of such a triangle, 
that is, we fixed a choice of the geodesics $[q r], [r p]$ and $[p q]$.
...
Note that any open set is locally convex by the definition.

\section{Length spaces}\label{sec:intrinsic}

A \emph{curve} is defined as a continuous map from a real interval to a space.
If the real interval is $[0,1]$ then curve is called \emph{path}.
...
the set $\cBall[x,\rho(x)-\eps]$ is a compact $\eps$-net in $B$. Since $B$ is closed and hence complete, it has to be compact.
\claimqeds
