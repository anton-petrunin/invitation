...
the angle $\alpha=\mangle\hinge q p x$ is defined.
Then
\[\dist{p}{\gamma(t)}{}
\le
...
Note that for a fixed sequence of metric spaces $(\spc{X}_n)$, it might be possible to construct different Gromov--Hausdorff convergences, say $\spc{X}_n\z{\xrightarrow{\rho}} \spc{X}_\infty$ and $\spc{X}_n\xrightarrow{\rho'} \spc{X}_\infty'$,   whose limit spaces $\spc{X}_\infty$ and $\spc{X}_\infty'$ need not be isometric to each other. 
