...
We can assume that the sequence $d_n=\distfun_{A_n}(x_\kay)$ is bounded for each~$\kay$.
...
we can assume that $\distfun_{A_n}(x_\kay)$ converges as $n\to\infty$ for any fixed~$\kay$.
...
\item the points $z_{1,n},\ldots,z_{M_\kay,n}$ form an $\tfrac1\kay$-net in $\cBall[x_n,\kay]_{\spc{X}_n}$.
\end{itemize}
