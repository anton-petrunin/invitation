...
\begin{itemize}
\item The \index{diameter}\emph{diameter} of metric space $\spc{X}$ is defined as
...
\parbf{Cone.}
The \index{cone}\emph{cone} $\spc{V}=\Cone\spc{U}$ over the metric space $\spc{U}$
is defined as the metric space whose underlying set consists of the %???do we need this ``the''?
...
\parbf{Suspension.}
The \index{suspension}\emph{suspension} $\spc{V}=\Susp\spc{U}$ over the metric space $\spc{U}$
...
A \index{geodesic path}\emph{geodesic path} is a geodesic with constant-speed parameterization by $[0,1]$.
...
%S:We need this in 3.2 and 3.3:
A curve $\gamma\:[0,1]\to \spc{X}$  is called a \index{geodesic!local geodesic}\emph{local geodesic path}, if for any $t\in\II$ there is a neighborhood $U$ of $t$ in $\II$ such that the restriction $\gamma|_U$ is a geodesic with a constant-speed parametrization. 
...
Let $\spc{X}$ be a metric space, 
$p,q,r\in \spc{X}$. 
Let us define its \index{model triangle}\emph{model triangle} $\trig{\~p}{\~q}{\~r}$ 
(briefly, 
...
$\trig{\~p}{\~q}{\~r}$ at $\~p$ 
will be called the \index{model angle}\emph{model angle} of triple $p$, $q$, $r$ and it will be denoted by
$\angk p q r_{\EE^2}$%
\index{$\tangle$!$\angk{{*}}{{*}}{{*}}$}.
...
%???the next sentence look strange, is it English??? 
...
Then the sequence of closed sets $(A_n)_{n=1}^\infty$ is said to \index{converge in the sense of Hausdorff}\emph{converge in the sense of Hausdorff}.
\end{thm}
...
In a  metric space $\spc{X}$, show that if $A_n\to A_\infty$ as in Definition \ref{def:hausdorff-coverge}, then $A_\infty$ is the set of all points $p$ such that   $p_n\to p$ for some sequence of points  $p_n\in\spc{X}$.

Show that the converse fails.
\end{thm} %S:trivial to do but I think  needed for understanding
