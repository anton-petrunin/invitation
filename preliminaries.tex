...
Set $A_\infty=f^{-1}(0)$. {\color{red} Let $a_n\in A_n$. Since $\spc{X}$ is proper and  $\dist{A_n}{}{}(x)$ is bounded by passing to a subsequence we can assume that $a_n\to a$. Clearly $a\in A_\infty$ and hence $A_\infty$ is non-empty.} 
...
It is clear that $f(z_\infty)=0$, that is, $z_\infty\in A_\infty$. {\color{red} (Note that this also shows that $A_\infty$ is non-empty).}
On the other hand, 
\begin{align*}
\dist{A_\infty}{}{}(y)
\le
