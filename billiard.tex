...
In this way we arrive at the definition of $\CAT(\kappa)$ comparison for any reл $\kappa$.
...
that is, the function $t\mapsto f\circ \gamma(t) - \tfrac 12\cdot t^2$ is convex for any unit-speed geodesic $\gamma$
\end{thm}
...
Show that $\spc{W}$ is $\CAT(0)$ if and only if 
$\spc{U}$ is $\CAT(0)$, and $A$ is convex in $\spc{U}$.
\end{thm}
