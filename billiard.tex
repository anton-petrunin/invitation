...
\textit{We assume that our trajectory never meets a wall in a tangent direction.
Also, we will always assume the trajectory meets at most one wall in a small neighborhood of every time moment.
For example, if $\gamma$ has to meet more than two walls at time moment $t$
...
It is straightforward to check that the metric defined by $\lVert {*}\rVert$, the collisions follow the standard law of billiards.
