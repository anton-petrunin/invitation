...
In particular, for any $r>0$ the set $A$ has a convex closed $r$-neighborhood
...
Let us construct the space $\spc{R}$ by gluing successive pairs of spaces along  $A^1,\dots,A^N$ respectively.
...
for some $x,y\in \EE^m$ and $i,j\in \{0,\dots,N\}$, then the necessarily unique geodesic $[\check x^i \check y^j]_{\check{\spc{R}}}$ is the image of the necessarily unique geodesic $[x^i y^j]_{\spc{R}}$ under the map $x^i\mapsto \check x^i$.
\end{thm}
...
Given two points $x,y\in \EE^m$, consider function $f(p)\df\dist{x}{p}{}+\dist{y}{p}{}$.
...
\textit{
We assume that our trajectory never meets a wall in a tangent direction.
...
Puff pastry was also used by Stephanie Alexander and Richard Bishop \cite{alexander-bishop}
...
It was shown by Dmitri Burago and Sergei Ivanov \cite{burago-ivanov} that the number of collisions that may occur between $n$ identical balls in $\RR^3$ grows at least exponentially in $n$; the two-dimensional case remains open.
