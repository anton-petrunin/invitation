...
and 
$\trig{\tilde q}{\tilde x}{\tilde y}=\modtrig{}(qxy)_{\EE^2}$ with common side $[\tilde x\tilde y]$ and lying on opposite sides of $[\tilde x\tilde y]$.
...
We can do the same for the model plane of curvature $\kappa$ for arbitrary $\kappa$;
...
In order to check $\CAT(\kappa)$ comparison for a quadruple of points, it is sufficient to know the 6 distances between all pairs of points in the quadruple.
...
Since the triangle $\trig{\gamma(0)}{\gamma(a)}{\gamma(b)}$ is thin (see the next section \footnote {\color{red} V: why refer to the next section here?}) and 
...
that is, the function $t\mapsto f\circ \gamma(t) - \tfrac 12\cdot t^2$ is convex for any unit-speed \footnote{\color{red} V: in our conventions all geodesics are unit speed } geodesic $\gamma$.
\end{thm}
...
Let $A$ be a convex closed set in a proper \footnote{\color{red} V: I don't think proper is needed here. } geodesic $\CAT(0)$ space $\spc{U}$;
...
The closest-point projection $p\mapsto p^*$ defined by \ref{SHORT.ex:closest-point:a} is short (i.e. 1-Lipschitz). 
\end{subthm}
...
If one applies this construction to two copies of one space $\spc{U}$ with a closed convex subset $A\subset \spc{U}$ and the identity map $\iota\:A\to A$, then the obtained space is called the \index{double}\emph{double} of $\spc{U}$ along~$A$.
...
Show that in the case of identical balls in the one-dimensional space (i.e. $\RR$) 
