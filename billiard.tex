...
Note that \textit{a metric space is proper if and only if the distance function from one (and therefore any) point is proper}; see Section~\ref{sec:metric spaces}.
...
Show that the doubling $\spc{W}$ of $\spc{U}$ along $A$ is $\CAT(0)$ if and only if $A$ is a convex set of $\spc{U}$.
\end{thm}
...
\textit{
We assume that trajectory never meet a wall in a tangent direction to $\partial A^i$. 
